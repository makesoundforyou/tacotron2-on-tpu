...
def prepare_directories_and_logger(output_directory, log_directory, rank):
    if rank == 0:
        ...
        logger = Tacotron2Logger(
            None)  # os.path.join(output_directory, log_directory))
    else:
        logger = None
    return logger
...
def validate(trainer, valset, iteration, batch_size, n_gpus, collate_fn,
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if not is_overflow and rank == 0:
                ...
                logger.log_training(reduced_loss, norm, trainer.lr, duration,
                                    iteration)
